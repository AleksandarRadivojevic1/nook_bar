...
import { displayTime, formatClock, formatDate, hours, isOpenAt, minuteOfDayInTz } from '../lib/hours';
...
export function startClock(root: ParentNode, locale: Locale): () => void {
  ...
  const tick = () => {
    ...
    if (statusEl) {
      const open = isOpenAt(minuteOfDayInTz(now), hours);
      statusEl.className = 'status ' + (open ? 'is-open' : 'is-shut');
